...
/**
 * Created by Dhillon on 21-11-2016.
 */

public class SearchMerchantWithPlace extends BaseSearchClass {
    ...
    public static List<SearchMerchantWithPlace> parseJson(String json) throws JSONException {
        JSONArray array = new JSONArray(json);
        List<SearchMerchantWithPlace> list = new ArrayList<>();
        if (array != null && array.length() > 0) {
            for (int i = 0; i < array.length(); i++) {
                JSONObject obj = array.getJSONObject(i);
                SearchMerchantWithPlace data = (SearchMerchantWithPlace) JsonUtil.parseJson(obj.toString(),
                        SearchMerchantWithPlace.class);
                list.add(data);
            }
        }
        return list;

...
public abstract class FusedLocationService implements GoogleApiClient.ConnectionCallbacks, GoogleApiClient.OnConnectionFailedListener {
    ...
    public static MyLocation getLatestLocation() {
        String lat = Preferences.getData(Preferences.KEY_LAT, null);
        String lng = Preferences.getData(Preferences.KEY_LNG, null);
        if (TextUtils.isEmpty(lat) || TextUtils.isEmpty(lng)) {
            return null;
        } else {
            MyLocation location = new MyLocation(lng, lat);
            return location;
        }
    }
